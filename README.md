...
Na tela inicial, selecione **Presencial** antes de criar a sala. Todos entram pelo código em um **saguão com lista de participantes em tempo real**. Nenhuma carta é sorteada nessa etapa (`deal: 0`). Com pelo menos duas pessoas, o anfitrião pode clicar em **Enviar cartas** quando a turma estiver pronta. Cada pessoa recebe uma carta grande de 1 a 100, com animação de giro. Toque na carta para esconder/mostrar o número. Após o primeiro envio, o único controle de jogo do anfitrião passa a ser **Reenviar cartas**. Não há tema, turnos, respostas, ordenação nem resultado nesse modo. Pessoas que entrarem após o envio recebem a carta da distribuição atual. Atualizar a página mantém o número; quem estava offline recebe a distribuição atual ao retornar. Salas antigas que já distribuíram cartas continuam na tela de carta; crie uma sala nova para começar pelo saguão.
